    ...
    private fun initInterfaces(iLoadAlert: ILoadAlert, iMessage: IMessage, iWorkingWithDataBase: IWorkingWithDataBase, directoryForDownloadFiles: File) {
        ...
        AppConfig.SystemClipboard.iSystemClipboard = object : ISystemClipboard {
            override fun setContent(o: Any) {
                Toolkit.getDefaultToolkit().systemClipboard.setContents(StringSelection(o as String), null)
            }

            override fun getContent(): Any = Toolkit.getDefaultToolkit().systemClipboard.getData(DataFlavor.stringFlavor)
        }
        AppConfig.ThreadType.iUnusualThread = object : IUnusualThread {
            override fun execute(iDelegateMethod: IDelegateMethod) {
                Platform.runLater { iDelegateMethod.voidMethod() }
            }
        }
        AppConfig.ThreadType.iUsualThread = object : IUsualThread {
            override fun execute(iDelegateMethod: IDelegateMethod) {
