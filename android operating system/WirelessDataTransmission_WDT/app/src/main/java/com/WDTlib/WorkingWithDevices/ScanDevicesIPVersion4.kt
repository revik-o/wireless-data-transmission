...
import com.WDTlib.DelegateMethods.IDelegateMethodIntegerArg
...
class ScanDevicesIPVersion4: IScanDevices {
    ...
    override fun startScanDevices() {
        AppConfig.IPWorkInterface.IPV4.iIP.getEnumerableListOfIP().forEach {
            AppConfig.DataBase.ModelDAOInterface.iDeviceModelDAO.selectWhereIPLike(it).forEach { DataTransfer.sendDataFromClient(InetSocketAddress(it[3], AppOption.Option.SOCKET_PORT), 0, actionMethodForFindDevice, localDeviceIPAndRepeatableIPsList) }
                        ...
                        if (this.isEnumeration) {
                            ...
                            if (InetAddress.getByName(temporaryIP).isReachable(timeOut)) {
                                ...
                                if (checkIP) {
                                    ...
                                    DataTransfer.sendDataFromClient(InetSocketAddress(temporaryIP, AppOption.Option.SOCKET_PORT), 0, actionMethodForFindDevice)
                                }
                            }
                            sum += i.toDouble() / 255
